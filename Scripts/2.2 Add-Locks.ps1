...
if(($subs -ne $null) -or ($subs.Count -gt 0))
{
    ...
    foreach($sub in $subs)
    {
        ...
        {
                New-AzureRmResourceLock -LockName DoNotDelete -ResourceGroupName $routeTable.ResourceGroupName -ResourceName $routeTable.Name -ResourceType $routeTable.Type -LockLevel CanNotDelete -LockNotes "Do Not delete lock" -Force
        }
